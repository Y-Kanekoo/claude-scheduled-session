...
GitHub Actions で5時間ごとにセッションを自動開始し、ウィンドウのリセットタイミングを制御する。追加の API 課金は発生しない（Max Plan サブスク内で動作）。
...
> **注**: GitHub Actions の cron は数分〜最大30分程度の遅延が発生する場合があります。ただし、レート制限はローリングウィンドウ（メッセージ送信時点から5時間）のため、多少のブレは問題ありません。ウィンドウの開始が数十分ズレるだけで、枠自体が消えることはありません。
