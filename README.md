...
GitHub Actions で5時間40分間隔でセッションを自動開始し、ウィンドウのリセットタイミングを制御する。追加の API 課金は発生しない（Max Plan サブスク内で動作）。
...
> **なぜ5時間ちょうどではなく5時間40分間隔か？**
> GitHub Actions の cron は最大30分程度の遅延が発生します。5時間ちょうどの間隔だと、遅延により前のウィンドウ内で次のセッションが実行されてしまう可能性があります。40分のバッファを設けることで、最大遅延を考慮しても確実に前のウィンドウ外で新しいセッションが開始されます。
